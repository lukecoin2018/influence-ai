...
// Latin American Spanish discover pages — handles both location and niche types
...
export async function generateStaticParams() {
  return getAllEsSlugs('es').map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const config = getEsPageConfig(slug, 'es');
  if (!config) return {};

  const url = `https://influenceit.app/es/discover/${slug}`;
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        ...
        'es': url,
        'es-ES': `https://influenceit.app/es-es/discover/${slug}`,
      },
    },
    openGraph: {
      title: config.title,
...
async function fetchCreators(config: ReturnType<typeof getEsPageConfig>, supabase: any) {
  ...
  if (config.type === 'location' && config.locationMatch) {
    ...
    if (error) console.error('[es/discover] location error:', error.message);
    return { creators: data ?? [], total: count ?? 0 };
  }

  if (config.type === 'niche' && config.searchKeyword) {
    // Build Spanish countries filter
    ...
    if (error) console.error('[es/discover] niche error:', error.message);
    return { creators: data ?? [], total: count ?? 0 };
  }
...
export default async function EsDiscoverPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Descubrir', item: 'https://influenceit.app/es/discover' },
      { '@type': 'ListItem', position: 3, name: config.title, item: `https://influenceit.app/es/discover/${slug}` },
    ],
  };
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-14 pb-12">
          <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
            ...
            <span>/</span>
            <a href="/es/discover" className="hover:text-gray-600 transition-colors">Descubrir</a>
            <span>/</span>
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-gray-200 bg-white text-xs font-semibold text-gray-600 mb-5">
            {config.platform === 'instagram' && (
              <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none">
                <defs>
                  <linearGradient id="ig-es" x1="0%" y1="100%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="#f09433" />
                    <stop offset="100%" stopColor="#bc1888" />
                  </linearGradient>
                </defs>
                <rect x="2" y="2" width="20" height="20" rx="5" fill="url(#ig-es)" />
