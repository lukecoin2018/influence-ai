...
// Deterministic avatar color based on first letter — no gradients, solid pastels
function getAvatarStyle(firstName: string): { bg: string; text: string } {
  const palettes = [
    { bg: '#FFE4E8', text: '#E8517A' },
    { bg: '#FFF3CD', text: '#D4820A' },
    { bg: '#E8F4FD', text: '#2E86C1' },
    { bg: '#E8F8F5', text: '#1A8F6F' },
    { bg: '#F3E8FF', text: '#8B5CF6' },
    { bg: '#FFF0E8', text: '#D4521A' },
    { bg: '#E8FFF3', text: '#1A8F4F' },
    { bg: '#F0F8FF', text: '#1A6BC1' },
  ];
  const idx = firstName.charCodeAt(0) % palettes.length;
  return palettes[idx];
}
...
export default function CreatorCard({ creator, signupUrl }: CreatorCardProps) {
  ...
  <span className="select-none blur-[4px]">
    {creator.firstName.slice(4).padEnd(4, 'xxxx').slice(0, Math.max(4, creator.firstName.length - 4))}
          ...
          <div className="flex flex-wrap gap-1.5">
            {creator.categories.map((cat) => (
              <span
                ...
                style={{ backgroundColor: '#FFF3CD', color: '#D4820A', border: '1px solid #FFE59A' }}
              >
                {cat}
              </span>
            ))}
      ...
      {/* View Full Profile button — matches brand style: gold/yellow outline */}
      <div className="px-5 pb-5">
        <a
         href={signupUrl}
         className="block w-full py-2.5 rounded-xl text-sm font-semibold text-center transition-colors hover:bg-[#FFD700]"
         style={{
