...
export default function AdminOverviewPage() {
  ...
  const statCard = (label: string, value: number | null, highlight = false) => (
    <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '20px 24px', border: '1px solid #E5E7EB' }}>
      <p style={{ fontSize: '28px', fontWeight: 800, color: highlight ? '#FFD700' : '#3A3A3A', margin: '0 0 4px 0' }}>
        {value ?? '—'}
      </p>
  ...
  return (
    <div>
      <h1 style={{ fontSize: '22px', fontWeight: 700, color: '#3A3A3A', margin: '0 0 24px 0', letterSpacing: '-0.02em' }}>
        Dashboard Overview
      </h1>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '12px', border: '1px solid #E5E7EB', padding: '24px' }}>
        <h2 style={{ fontSize: '14px', fontWeight: 700, color: '#3A3A3A', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 20px 0' }}>
          Recent Activity
        </h2>
        {activity.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {activity.map((event) => (
              <div key={event.id} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#FFD700', flexShrink: 0 }} />
