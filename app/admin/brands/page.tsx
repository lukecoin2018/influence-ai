              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                <div>
                  ...
                  <p style={{ fontSize: '12px', color: '#9CA3AF', margin: 0 }}>
                    Signed up: {new Date(brand.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                  </p>
                </div>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  {brand.approval_status === 'approved' && (
                    <button
                      onClick={() => updateStatus(brand.id, 'suspended')}
                      disabled={actionLoading === brand.id + 'suspended'}
                      style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', border: 'none', backgroundColor: '#F3F4F6', color: '#6B7280' }}
                    >
                      🚫 Suspend
                    </button>
                  )}
